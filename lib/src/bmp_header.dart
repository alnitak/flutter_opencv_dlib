...
class Bmp32Header {
  ...
  Bmp32Header.setHeader(this.width, this.height) {
    ...
    bd.setUint16(0x1C, 32, Endian.little);
  ...
  Uint8List storeBitmap(Uint8List bitmap, {bool isGreyscale = false}) {
    if (!isGreyscale) {
      bmp.setRange(rgba32HeaderSize, contentSize * bytesPerPixel, bitmap);
    } else {
      for (int i=0; i<bitmap.length; ++i) {
        bmp[rgba32HeaderSize + i*4 + 0] = bitmap[i];
        bmp[rgba32HeaderSize + i*4 + 1] = bitmap[i];
        bmp[rgba32HeaderSize + i*4 + 2] = bitmap[i];
        bmp[rgba32HeaderSize + i*4 + 3] = 0xff;
      }
    }
    return bmp;
  }
